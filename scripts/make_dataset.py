...
def load_compounds(path):
    df = pd.read_csv(path)
    c = ci_map(df)
    drug = c.get("drug id") or c.get("drug_id") or "Drug id"
    smi  = c.get("smiles") or "SMILES"
    out = df[[drug, smi]].drop_duplicates()
...
def load_cells(path):
    ...
    gdsc_t1 = cols.get("gdsc tissue descriptor 1")
    gdsc_t2 = cols.get("gdsc tissue descriptor 2")
    ...
    if gdsc_t1 or gdsc_t2:
        ...
        print("WARNING load_cells: no tissue columns found; setting tissue='unknown'.")
        out["tissue"] = "unknown"
...
def process_expression_zip(expr_zip, topk=5000, pcs=512):
    # GDSC1000: genes x cell_lines (or the reverse) tsv inside a zip
...
def main(args):
    ...
    # 4) Basic cleaning - drop missing IC50; canonicalize SMILES
    resp = resp.dropna(subset=["LN_IC50"])
    resp["SMILES"] = resp["SMILES"].astype(str).replace({"nan": np.nan})
    resp["SMILES"] = resp["SMILES"].apply(canonical_smiles)
    resp["SMILES"] = resp["SMILES"].fillna("")  # keep unmatched drugs
    ...
    # 5) Expression PCs
    expr_zip = os.path.join(raw, "Cell_line_RMA_proc_basalExp.txt.zip")
